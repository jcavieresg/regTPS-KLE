...
run_tmb_spde <- function(N_sp, dim_grid, sp_points, mesh, y_obs, u_true, u_grid, Cov_true) {
  ...
  res_list <- list(obj = obj_spde, opt = opt_spde, rep = rep_spde, tmb_data = tmb_data, tmb_par = tmb_par,
                   mesh = mesh, spde = spde, u_true_sp = u_true, u_true_grid = u_grid, Cov_true = Cov_true)
  return(res_list)
}
...
#====================================================
# Creating the lists for the SPDE/regTPS-KLE models
#====================================================
fits_TMB_spde <- list()
fits_TMB_tps <- list()
...
for (i in 1:n_scenarios) {
  ...
  Cov_true <- matern_cov(mesh$loc, nu = nu, rho = rho0, sigma2 = sigma_u0^2)
