...
# Reading the outputs
# TMB models
tmb_spde <- readRDS('fits_TMB_spde_expo.RDS')
tmb_tps <- readRDS('fits_TMB_tps_expo.RDS')
...
# MCMC models (Stan)
mcmc_spde1 <- readRDS('stan_spde_expo_1.RDS')
mcmc_spde2 <- readRDS('stan_spde_expo_2.RDS')
mcmc_spde3 <- readRDS('stan_spde_expo_3.RDS')
mcmc_spde4 <- readRDS('stan_spde_expo_4.RDS')

mcmc_tps1 <- readRDS('stan_tps_expo_1.RDS')
mcmc_tps2 <- readRDS('stan_tps_expo_2.RDS')
mcmc_tps3 <- readRDS('stan_tps_expo_3.RDS')
mcmc_tps4 <- readRDS('stan_tps_expo_4.RDS')
...
ggsave(filename = "C:/Users/jcavi/OneDrive/Escritorio/KLE/plots/plot_post_fields_expo.pdf",
...
# Save as high-quality PDF
ggsave(filename = "C:/Users/jcavi/OneDrive/Escritorio/KLE/plots/plot_post_field_diff_expo.pdf",
...
metrics_long %>%
  kable(
    ...
    booktabs = TRUE,
    digits = 3,
    caption = "Comparison of SPDE and regTPS-KLE models with the true GRF using a Exponential covariance function across scenarios.",
...
#=================================
#   RMSE, R², and MAE Computation
#=================================

# Function to compute all metrics for a single scenario and method
compute_metrics <- function(predicted, true) {
  # Ensure vectors
  pred_vec <- as.vector(predicted)
  true_vec <- as.vector(true)
  
  # Remove any NA values
  valid_idx <- !is.na(pred_vec) & !is.na(true_vec)
  pred_vec <- pred_vec[valid_idx]
  true_vec <- true_vec[valid_idx]
  
  # Compute metrics
  rmse <- sqrt(mean((pred_vec - true_vec)^2))
  r2   <- cor(pred_vec, true_vec)^2
  mae  <- mean(abs(pred_vec - true_vec))
  
  # Additional useful metrics
  bias <- mean(pred_vec - true_vec)
  
  return(c(RMSE = rmse, R2 = r2, MAE = mae, Bias = bias))
}

# Compute metrics for all scenarios
n_scenarios <- 4
metrics_list <- vector("list", n_scenarios)

for (i in 1:n_scenarios) {
  # SPDE metrics
  spde_metrics <- compute_metrics(
    predicted = spde_field_grid[[i]],
    true      = tmb_spde[[i]][[9]]  # Adjust index if needed
  )
  
  # regTPS-KLE metrics
  tps_metrics <- compute_metrics(
    predicted = tps_field_grid[[i]],
    true      = tmb_tps[[i]][[11]]   # Adjust index if needed
  )
  
  # Create comparison table
  metrics_list[[i]] <- data.frame(
    Metric = c("RMSE", "R²", "MAE", "Bias"),
    SPDE   = spde_metrics,
    regTPS = tps_metrics,
    Difference = spde_metrics - tps_metrics,  # Positive means SPDE worse
    row.names = NULL
  )
  
  # Add relative performance (percentage)
  metrics_list[[i]]$Relative_Diff_Pct <- 
    round(100 * (spde_metrics - tps_metrics) / abs(spde_metrics), 2)
}
...
for (i in 1:n_scenarios) {
  ...
  print(metrics_list[[i]], row.names = FALSE, digits = 4)
  
  # Determine winner for each metric
  cat("\nBest performer:\n")
  for (metric_idx in 1:3) {  # Only for RMSE, R², MAE (not Bias)
    metric_name <- metrics_list[[i]]$Metric[metric_idx]
    spde_val <- metrics_list[[i]]$SPDE[metric_idx]
    tps_val <- metrics_list[[i]]$regTPS[metric_idx]
    
    if (metric_name == "R²") {
      winner <- ifelse(spde_val > tps_val, "SPDE", "regTPS-KLE")
    } else {
      winner <- ifelse(spde_val < tps_val, "SPDE", "regTPS-KLE")
    }
    cat(sprintf("  %s: %s\n", metric_name, winner))
...
#=================================
cat("\n", paste(rep("=", 70), collapse = ""), "\n")
cat("                 SUMMARY ACROSS ALL SCENARIOS\n")
cat(paste(rep("=", 70), collapse = ""), "\n\n")

# Aggregate metrics
summary_df <- data.frame(
  Metric = c("RMSE", "R²", "MAE", "Bias"),
  SPDE_Mean = rowMeans(sapply(metrics_list, function(x) x$SPDE)),
  SPDE_SD   = apply(sapply(metrics_list, function(x) x$SPDE), 1, sd),
  regTPS_Mean = rowMeans(sapply(metrics_list, function(x) x$regTPS)),
  regTPS_SD   = apply(sapply(metrics_list, function(x) x$regTPS), 1, sd)
)

print(summary_df, row.names = FALSE, digits = 4)

# Overall winner count
cat("\n\nOverall winner count (across all scenarios):\n")
winner_count <- c(SPDE = 0, regTPS = 0)

for (i in 1:n_scenarios) {
  for (metric_idx in 1:3) {
    metric_name <- metrics_list[[i]]$Metric[metric_idx]
    spde_val <- metrics_list[[i]]$SPDE[metric_idx]
    tps_val <- metrics_list[[i]]$regTPS[metric_idx]
    
    if (metric_name == "R²") {
      if (spde_val > tps_val) winner_count["SPDE"] <- winner_count["SPDE"] + 1
      else winner_count["regTPS"] <- winner_count["regTPS"] + 1
    } else {
      if (spde_val < tps_val) winner_count["SPDE"] <- winner_count["SPDE"] + 1
      else winner_count["regTPS"] <- winner_count["regTPS"] + 1
    }
...
print(winner_count)
cat(sprintf("\nSPDE won in %.1f%% of comparisons\n", 
            100 * winner_count["SPDE"] / sum(winner_count)))

#=================================
# Optional: Create publication-ready table
#=================================
library(knitr)  # Load if available

# Combine all scenarios into one table
combined_table <- do.call(rbind, lapply(1:n_scenarios, function(i) {
  df <- metrics_list[[i]][1:3, ]  # Only RMSE, R², MAE
  df$Scenario <- i
  df <- df[, c("Scenario", "Metric", "SPDE", "regTPS")]
  return(df)
}))

# Format for publication
combined_table$SPDE <- sprintf("%.4f", combined_table$SPDE)
combined_table$regTPS <- sprintf("%.4f", combined_table$regTPS)
...
if (require(knitr, quietly = TRUE)) {
  cat("\n\nLaTeX format:\n")
  print(kable(combined_table, format = "latex", booktabs = TRUE))
}
...
metrics_to_plot <- c("RMSE", "R²", "MAE")
colors <- c("SPDE" = "#E41A1C", "regTPS" = "#377EB8")

for (metric in metrics_to_plot) {
  metric_idx <- which(metrics_list[[1]]$Metric == metric)
  
  spde_vals <- sapply(metrics_list, function(x) x$SPDE[metric_idx])
  tps_vals <- sapply(metrics_list, function(x) x$regTPS[metric_idx])
  
  # Determine y-axis direction
  if (metric == "R²") {
    ylim <- c(min(c(spde_vals, tps_vals)) - 0.05, 1)
    better_direction <- "higher"
  } else {
    ylim <- c(0, max(c(spde_vals, tps_vals)) * 1.1)
    better_direction <- "lower"
  }
  
  plot(1:n_scenarios, spde_vals, type = "b", col = colors["SPDE"], 
       lwd = 2, pch = 19, ylim = ylim,
       xlab = "Scenario", ylab = metric, 
       main = paste(metric, "(", better_direction, "is better)"),
       xaxt = "n")
  lines(1:n_scenarios, tps_vals, type = "b", col = colors["regTPS"], 
        lwd = 2, pch = 17)
  axis(1, at = 1:n_scenarios)
  grid()
  legend("topright", legend = c("SPDE", "regTPS-KLE"), 
         col = colors, lwd = 2, pch = c(19, 17), bty = "n")
}

# Add bias plot
metric_idx <- which(metrics_list[[1]]$Metric == "Bias")
spde_vals <- sapply(metrics_list, function(x) x$SPDE[metric_idx])
tps_vals <- sapply(metrics_list, function(x) x$regTPS[metric_idx])

plot(1:n_scenarios, spde_vals, type = "b", col = colors["SPDE"], 
     lwd = 2, pch = 19, ylim = range(c(spde_vals, tps_vals)),
     xlab = "Scenario", ylab = "Bias", 
     main = "Bias (closer to 0 is better)",
     xaxt = "n")
lines(1:n_scenarios, tps_vals, type = "b", col = colors["regTPS"], 
      lwd = 2, pch = 17)
...
plot4 <- ggplot(all_data, aes(x = Var1, y = Var2, fill = value)) +
  geom_tile() +
...
fits_TMB_spde <- tmb_spde
fits_TMB_tps <- tmb_tps
...
process_scenario <- function(scenario_number) {
  ...
  plot_data_diff_tps <- prep_data(diff_tps_true, "regTPS-KLE - Exp. Cov True", scenario_number)
  plot_data_diff_spde <- prep_data(diff_spde_true, "SPDE - Exp. Cov True", scenario_number)
...
filter(combined_all_data, model == "regTPS-KLE - Exp. Cov True")
filter(combined_all_data, model == "SPDE - Exp. Cov True")



# Plot the combined heatmaps with facet_grid and free scales
combined_all_data$model <- factor(combined_all_data$model, levels = c("SPDE - Exp. Cov True", "regTPS-KLE - Exp. Cov True"))

plot8 <- ggplot(combined_all_data, aes(x = s1_idx, y = s2_idx, fill = scaled_value)) +
  geom_tile() +
  facet_grid(scenario ~ model, scales = "free") +
  scale_fill_gradient2(
    low = "blue", high = "red", mid = "white",
    midpoint = 0, name = "Scaled\nDifference") +
  labs(title = "Comparison of Exp. Covariance Model Differences",
...
#===================================================
# Compute metrics - IMPROVED with function
#===================================================
compute_metrics <- function(predicted, true) {
  rmse <- sqrt(mean((predicted - true)^2))
  r2   <- cor(as.vector(predicted), as.vector(true))^2
...
for (i in 1:4) {
  # Get true field
  u_true_grid <- tmb_tps[[i]]$u_true_grid
  
  # SPDE metrics
  spde_metrics <- compute_metrics(spde_field_grid[[i]], u_true_grid)
  
  # TPS metrics
  tps_metrics <- compute_metrics(tps_field_grid[[i]], u_true_grid)
  
  # Combine
  results_list[[i]] <- data.frame(
    Scenario = paste0("Sce. ", i),
    Method = c("SPDE", "regTPS-KLE"),
    rbind(spde_metrics, tps_metrics)
  )
}
...
kable(results_table, format = "latex", booktabs = TRUE, 
      caption = "Comparison of SPDE and regTPS-KLE models")
